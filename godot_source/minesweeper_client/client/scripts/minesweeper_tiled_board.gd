...
func _ready():
	...
	get_parent().win.connect(pass_win)
	get_parent().draw.connect(pass_draw)
	get_parent().lose.connect(pass_lose)
...
func pass_input(tile_position, input_type) -> void:
	input.emit(tile_position, input_type)
...
func pass_opponent_tile_uncovered(tile_number) -> void:
...
func fit_to_window() -> void:
	if !((get_window().size.y * board_width) > (get_window().size.x * board_height)):
		scale = Vector2(get_window().size.y / float(board_height * tile_size_px),get_window().size.y / float(board_height * tile_size_px))
	else:
		scale = Vector2(get_window().size.x / float(board_width * tile_size_px),get_window().size.x / float(board_width * tile_size_px))
	
	var x_offset = (get_window().size.x)/2 - scale.x * board_width * tile_size_px/2
